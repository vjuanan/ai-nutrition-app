...
export function NutritionalPlanWizard({ isOpen, onClose }: NutritionalPlanWizardProps) {
    ...
    // Or stick to "Acumulación" etc if user prefers training terms, but "Focus" field is generic.
    // Let's use generic "Focus" placeholders.
    const [weeklyLabels, setWeeklyLabels] = useState<string[]>(() => {
        const defaults = Array(4).fill('Mantenimiento');
        return defaults;
    });
    ...
    const handleSubmit = async () => {
        ...
        try {
            const result = await createNutritionalPlan(planName, null, {
    ...
    return createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative w-full max-w-3xl mx-4 bg-cv-bg-primary border border-cv-border rounded-2xl shadow-cv-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
                {/* Header - Compact */}
                <div className="relative px-5 py-3 border-b border-cv-border bg-gradient-to-r from-green-500/10 via-emerald-500/5 to-transparent shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-cv-text-tertiary">
                                Define la estructura macro de la dieta
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="px-6 py-4 space-y-4 overflow-y-auto">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Ej: High Protein - Hypertrophy Block"
                            className="w-full px-3 py-2 text-sm rounded-lg bg-cv-bg-secondary border border-cv-border text-cv-text-primary placeholder:text-cv-text-tertiary focus:outline-none focus:ring-2 focus:ring-green-500/50 focus:border-green-500 transition-all"
                            autoFocus
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            placeholder="Ej: Superávit calórico controlado (+300kcal)"
                            rows={2}
                    ...
                    <div className="grid grid-cols-12 gap-4">
                        ...
                        <div className="col-span-3">
                            ...
                            <div className="flex items-center gap-2">
                                <input
                                    ...
                                    min={1}
                                    max={12}
                                    className="w-full px-3 py-2 text-sm rounded-lg bg-cv-bg-secondary border border-cv-border text-cv-text-primary text-center focus:outline-none focus:ring-2 focus:ring-green-500/50 focus:border-green-500 transition-all"
                                />
                        ...
                        <div className="col-span-5">
                            ...
                            <input
                                ...
                                className="w-full px-3 py-2 text-sm rounded-lg bg-cv-bg-secondary border border-cv-border text-cv-text-primary focus:outline-none focus:ring-2 focus:ring-green-500/50 focus:border-green-500 transition-all"
                            />
                        </div>
                    ...
                    <div className="px-3 py-2 rounded-lg bg-gradient-to-r from-cv-bg-secondary to-cv-bg-tertiary border border-cv-border">
                        <div className="flex items-center justify-between text-[10px] text-cv-text-tertiary mb-1.5">
                            ...
                            <span className="text-green-500 font-medium">{duration} semanas</span>
                    ...
                    <div>
                        <label className="block text-xs font-medium text-cv-text-secondary mb-2">
                            Fase/Objetivo Semanal (Opcional)
                        </label>
                            ...
                            {Array.from({ length: duration }).map((_, i) => (
                                <div key={i} className="flex flex-col gap-1">
                                    ...
                                    <StrategyInput
                                        ...
                                        placeholder="Focus"
                                        className="w-full px-2 py-1.5 text-xs rounded-md bg-cv-bg-secondary border border-cv-border text-cv-text-primary placeholder:text-cv-text-tertiary/50 focus:outline-none focus:ring-1 focus:ring-green-500/50 transition-all"
                                    />
                                </div>
                            ))}
                        </div>
