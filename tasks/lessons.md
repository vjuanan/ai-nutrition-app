...
- [x] **Next.js 14 $$id Redefine Bug**: NUNCA usar `export const alias = existingServerAction` en archivos `'use server'`. Next.js llama `registerServerReference` en cada export, y al reasignar la misma función a un nuevo nombre, intenta redefinir `$$id` en el mismo objeto, causando `TypeError: Cannot redefine property: $$id`. **Solución**: Crear wrapper functions (`export async function alias() { return original(); }`) en lugar de aliases directos.
- [x] **CRÍTICO - URL de Producción**: La URL de producción de AI Nutrition es **https://ainutrition.epnstore.com.ar** — NUNCA usar https://aicoach.epnstore.com.ar (esa es la app de entrenamiento, otro proyecto). SIEMPRE verificar cambios en `ainutrition`, no en `aicoach`.
